...
public class Day05 {
  ...
  private record SeedLine(List<Long> seeds) {
    public static SeedLine from(List<String> input) {
      var seeds = input.stream()
          .limit(1)
          .flatMap(firstLine -> Arrays.stream(firstLine.split(":")).skip(1).map(line -> line.trim()))
          .flatMap(seedsLine -> {
  ...
  private record CategoryRange(long destinationStart, long sourceStart, long rangeLength) {
    ...
    public static CategoryRange from(String line) {
      var rangeValues = Arrays.stream(line.split("\\s+"))
          .map(Long::valueOf)
          .toArray(Long[]::new);
  ...
  public static long part1(List<String> input) {
    ...
    var locations = seedLine.seeds().stream()
  ...
  public static long part2(List<String> input) {
    ...
    var a = seedRanges.stream().map(range -> {
      long location;
