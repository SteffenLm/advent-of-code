 ...
 * <a href="https://adventofcode.com/2023/day/01">AoC page</a>.
 */
public class Day01 {
    private final static int NUMERIC_ONE = (int) '1';
    private final static int NUMERIC_NINE = (int) '9';
    private final static String[] STRING_NUMBERS = {
            ...
            for (String stringNumber : STRING_NUMBERS) {
                if (!firstCharIsNumber) {
                    ...
                    if (firstOccurenceIndex != -1
                            && (firstOccurenceMatch == null || firstOccurenceIndex < firstOccurenceMatch.index())) {
                        firstOccurenceMatch = new Match(firstOccurenceIndex, getNumericValue(stringNumber));
                    }
                }
                ...
                if (!lastCharIsNumber) {
                    ...
                    if (lastOccurenceIndex != -1
                            && (lastOccurenceMatch == null || lastOccurenceIndex > lastOccurenceMatch.index())) {
                        lastOccurenceMatch = new Match(lastOccurenceIndex, getNumericValue(stringNumber));
                    }
                }
            }
            String numberOne = firstCharIsNumber ? String.valueOf(line.charAt(0))
                    : firstOccurenceMatch != null ? String.valueOf(firstOccurenceMatch.value()) : "";
            String numberTwo = lastCharIsNumber ? String.valueOf(line.charAt(line.length() - 1))
                    : lastOccurenceMatch != null ? String.valueOf(lastOccurenceMatch.value()) : "";
            return numberOne + numberTwo;
        }).mapToInt(line -> Integer.valueOf(line)).sum();
    }
