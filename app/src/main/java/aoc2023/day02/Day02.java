...
/**
 * My solution for day 02 of Advent of Code 2023.
 * The puzzle can be found at the
 * <a href="https://adventofcode.com/2023/day/02">AoC page</a>.
 */
public class Day02 {
    ...
    record Cube(int amount, Color color) {
        ...
        private static Cube from(String... cubeValues) {
            return new Cube(
                    Integer.valueOf(cubeValues[0]),
                    Color.from(cubeValues[1]));
        }
    }
    ...
    record CubeSet(int red, int green, int blue) {
        ...
        public static BinaryOperator<CubeSet> accumulate = (acc, cur) -> new CubeSet(
                acc.red() + cur.red(),
                acc.green() + cur.green(),
                acc.blue() + cur.blue());

        public static BinaryOperator<CubeSet> accumulateMinimalValues = (acc, cur) -> new CubeSet(
    ...
    record Game(int id, List<CubeSet> sets) {
        public boolean allSetsMatch(Predicate<CubeSet> validator) {
            return this.sets.stream()
                    .filter(set -> !set.validate(validator))
                    .findAny()
                    .isEmpty();
        }
    ...
    public static int part2(List<String> input) {
        return input.stream()
                .map(line -> Game.from(LineParts.from(line), lineParts -> CubeSet.aggregateMinimalCubeSet(lineParts)))
                .map(game -> game.sets().stream()
                        .map(cube -> cube.red() * cube.green() * cube.blue())
                        .mapToInt(cubeSum -> cubeSum)
                        .sum())
                .mapToInt(gameSum -> gameSum).sum();
    }

    private static Predicate<CubeSet> part1Validator = cubeset -> cubeset.red() <= 12
            && cubeset.green() <= 13 && cubeset.blue() <= 14;
}
